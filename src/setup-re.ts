...
const PACKAGE_NAME: string = 're-build-system';
...
function getURL(
  ...
  if (matching_assets == undefined) {
    ...
    throw new Error(
      `Could not find ${process.platform} asset for Re version ${version.name}`
    );
  }
...
export async function addReToToolCache(
  version: vi.VersionInfo,
  arch_candidates: Array<string>
): Promise<string> {
...
async function getBinDirectoryFrom(tool_path: string): Promise<string> {
  // TODO: this will likely not work on macOS!
  return tool_path;
...
export async function addReToPath(
  version: vi.VersionInfo,
  arch_candidates: Array<string>
): Promise<void> {
  let tool_path: string = tc.find(PACKAGE_NAME, version.name);
  if (!tool_path) {
    tool_path = await addReToToolCache(version, arch_candidates);
  }
